...
export function Card(props: { card: CardModel, expandable: boolean, columnNumber?: number, rowNumber?: number, unclickable?: boolean }) {
  ...
  return (
    <div className="card" onClick={() => { if (!props.card.hidden && props.unclickable !== true) dispatch(moveCardTableau({ column: props.columnNumber || 0, row: props.rowNumber || 0 })) }}>
      <img src={img} alt="The Card" className={"card-image" + (props.expandable ? " expandable" : "")} />
    </div>
  );
}
